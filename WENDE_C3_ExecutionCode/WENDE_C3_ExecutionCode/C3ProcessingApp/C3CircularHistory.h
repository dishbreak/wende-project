...
class C3CircularHistory
{
	private:
		...
		unsigned int m_size;
		// the history buffer
		vector<C3_TRACK_POINT_DOUBLE> m_history;
	...
	public:
		...
		C3_TRACK_POINT_DOUBLE operator [] (const unsigned int index);

	public:
		// add a point to the circular buffer
		void addPosition(const C3_TRACK_POINT_DOUBLE cameraRoverPosition);
	...
	public:
		...
		C3_TRACK_POINT_DOUBLE getPoint(const unsigned int index ) const;
		//
};
