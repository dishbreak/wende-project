...
void servoSweep()
{
  ...
  myservoAz.writeTicks(1900);  // Min Az position
  ...
  myservoEl.writeTicks(3900); // Max El position
  myservoAz.writeTicks(4100); // Max Az position
...
void azServoRes()
{
  ...
  for(temp = 0; temp < 15; temp += 1)
  {
    ...
    myservoAz.writeTicks(3000+temp);
    delay(4000);
  }
...
void elServoRes()
{
  ...
  for(temp = 0; temp < 1000; temp += 1)
  {
    // Move the Elevation servo by 1 step and wait 4 seconds
    myservoEl.writeTicks(3548-temp);
    delay(40);
  }
