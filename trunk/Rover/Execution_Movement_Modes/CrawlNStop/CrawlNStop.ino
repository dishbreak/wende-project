...
void loop() {
  while (lastMilli <=2000) { //insert random variable here for while timer
  ...
  digitalWrite(RM_dir, !Forward);
  delay(1000);
  LMspeedRef = 50; //for slow, 200 for fast
  RMspeedRef = 50; //for slow, 200 for fast
  loopTime = 200; // for slow, 100 for fast
  Kp = 0.15; // proportional gain value
  Ki = .0009; // integrative gain value
  Kd = 0; // derivative gain value
  LM_count = 0;
  RM_count = 0;
  oldLMcount = 0;
  oldRMcount = 0;
  ...
  while (lastMilli <=20000) {  //appropriate delay for stop distance (or compare count)
    if (millis()-lastMilli >= loopTime) {
      lastMilli = millis();
      calcSpeed();
      LM_PWM = calcPWM(LM_PWM, LMspeedRef, LMspeedMeas);
