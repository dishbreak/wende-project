...
//				serverInit();	//server must be initialized and started by calling method.
				p("NetStart - SERVER FUNCTION CALLED");
//				server();
				break;
			case 1:
				clientInit();
//				client();		// client must be started by calling method
				break;
			default:
				p("Something went wrong, did not recieve correct role");
	...
	private void clientInit() throws IOException {
		...
		while(cSocket == null)
		{
			...
			try {
				...
				p(tries + " -> Client connecting to "+host+" on port "+port);
...
//				clientInputStream = new ObjectInputStream(cSocket.getInputStream());
				...
				clientInputStream = cSocket.getInputStream();
	...
	private int readInt() throws IOException {
		int inSize = 0;
		try {
		clientInputStream.read(inBytes,0,4);
		//p("\t"+inBytes[0]+","+inBytes[1]+","+inBytes[2]+","+inBytes[3]);
		inSize = byteArrayToInt(inBytes);
		//p("inSize ->" + inSize);
		return inSize;
		}
		catch(NullPointerException e) {
			p("Caught Null Pointer");
			return inSize;
		}
		catch(IOException e) {
...
//			System.out.println("msgType = " + msgType + " Insize = " + inSize);
			...
			clientInputStream.read(bytes,0,inSize); 		// Read protobuf serialized data from input stream
...
//			switch (msgType) {						// Select which protobuf message parser to use
			switch (port-4444) {						// Select which protobuf message parser to use
				case 0:
				...
				case 2:
					...
					break;
				default:
					System.err.println("message Type error " + msgType);
					break;
			}
