...
%Capture 10 frames and extract the last one as the background 
while(n~=11)
...
while(n~=700)
    ...
    if TestMode
        imshow(CameraSystem.FR)
        hold on
        CameraSystem.PlotDetection(CameraSystem.col,CameraSystem.row,'r','o')
    end
    T_4 = tic;
    ...
    if TestMode
        CameraSystem.PlotDetection(CameraSystem.colObj1,CameraSystem.rowObj1,'g','o')
        CameraSystem.PlotDetection(CameraSystem.colObj2,CameraSystem.rowObj2,'g','o')
        hold off
    end
    n = n + 1;
   CameraSystem.FlushCameraData
